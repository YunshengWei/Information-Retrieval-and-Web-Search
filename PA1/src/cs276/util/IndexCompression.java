...
public class IndexCompression {
  ...
  public static int VBEncodeInteger(int gap, byte[] outputVBCode) {
    ...
    int n = gap;
    while (true) {
      outputVBCode[outputVBCode.length - ++numBytes] = (byte) (n % 128);
      if (n < 128) {
        break;
      }
      n = n / 128;
    }
    outputVBCode[outputVBCode.length - 1] += 128;
    for (int i = 0; i < numBytes; i++) {
      outputVBCode[i] = outputVBCode[i + outputVBCode.length - numBytes];
    }
    return numBytes;
  }
  ...
  public static void VBDecodeInteger(byte[] inputVBCode, int startIndex, int[] numberEndIndex) {
    ...
    for (int i = startIndex; i < inputVBCode.length; i++) {
      if (inputVBCode[i] < 0) {
        n = n * 128 + (inputVBCode[i] - (byte) 128);
        numberEndIndex[0] = n;
        numberEndIndex[1] = i + 1;
        return;
      } else {
        n = n * 128 + inputVBCode[i];
      }
    }
  ...
  public static int unaryEncodeInteger(int number, BitSet outputUnaryCode, int startIndex) {
    ...
    outputUnaryCode.set(startIndex + number, false);
    nextIndex = startIndex + number + 1;
    return nextIndex;
  }
  ...
  public static void unaryDecodeInteger(BitSet inputUnaryCode, int startIndex, int[] numberEndIndex) {
    // TODO: Fill in your code here
    int endIndex = inputUnaryCode.nextClearBit(startIndex);
    numberEndIndex[0] = endIndex - startIndex;
    numberEndIndex[1] = endIndex + 1;
  }
  ...
  public static int gammaEncodeInteger(int number, BitSet outputGammaCode, int startIndex) {
    ...
    // TODO: Fill in your code here
    if (number > 0) {
      int offsetLength = (int) (Math.floor(Math.log(number) / Math.log(2)));
      startIndex = IndexCompression.unaryEncodeInteger(offsetLength, outputGammaCode, startIndex);
      nextIndex = startIndex + offsetLength;
      for (int i = nextIndex - 1; i >= startIndex; i--) {
        outputGammaCode.set(i, number % 2 != 0);
        number /= 2;
      }
