...
public class CosineSimilarityScorer extends AScorer {
  ...
  public double getNetScore(Map<String, Map<String, Double>> tfs, Query q,
    ...
    score += urlweight * AScorer.innerProduct(tfs.get("url"), tfQuery);
    score += titleweight * AScorer.innerProduct(tfs.get("title"), tfQuery);
    score += bodyweight * AScorer.innerProduct(tfs.get("body"), tfQuery);
    score += headerweight * AScorer.innerProduct(tfs.get("header"), tfQuery);
    score += anchorweight * AScorer.innerProduct(tfs.get("anchor"), tfQuery);
  ...
  public void normalizeTFs(Map<String, Map<String, Double>> tfs, Document d,
    ...
    for (Map<String, Double> tf : tfs.values()) {
      for (Map.Entry<String, Double> e : tf.entrySet()) {
        e.setValue(e.getValue() / (d.body_length + smoothingBodyLength));
      }
    }
  }
